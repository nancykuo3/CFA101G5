...
public class compositeQuery_ProdInfo {
	public static String get_aCondition(String columnName, String value) {

		String aCondition = null;

//		if ("isbn".equals(columnName) || "prodName".equals(columnName) || "prodLang".equals(columnName)
//				|| "prodVer".equals(columnName)) { // 用於varchar
//			aCondition = columnName + " like '%" + value + "%'";
//		}
		
		if("isbn".equals(columnName)) {
			aCondition = "ISBN like '%" + value + "%'";
		} else if ("prodName".equals(columnName)) {
			aCondition = "PROD_NAME like '%" + value + "%'";
		} else if("prodLang".equals(columnName)) {
			aCondition = "PROD_LANG like '%" + value + "%'";
		} else if("prodVer".equals(columnName)) {
			aCondition = "PROD_VER like '%" + value + "%'";
		}
		return aCondition + " ";
	}
	...
	public static String get_WhereCondition(Map<String, String[]> map) {
		...
		for (String key : keys) {
			...
			if (value != null && value.trim().length() != 0 && !"action".equals(key)) {
				...
				System.out.println("送出查詢資料的欄位數：" + count);
			}
		}
