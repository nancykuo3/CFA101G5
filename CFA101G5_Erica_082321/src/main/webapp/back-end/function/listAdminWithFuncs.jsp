...
List<FunctionVO> list = adminSvc.getFuncsByAdminid(adminVO.getAdminid());
        ...
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0">
            <div class="container-fluid d-flex p-0">
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    ...
                    <li class="nav-item"><a class="nav-link" href="<%=request.getContextPath()%>/back-end/backup1.html"><i class="fas fa-table"></i>備用1</a></li>
                    <li class="nav-item"><a class="nav-link" href="<%=request.getContextPath()%>/back-end/backup2.html"><i class="fas fa-table"></i><span>備用2</span></a></li>
                </ul>
					...
					<form method="post" id="adminFunc" 
						...
						<table class="table table-striped table-sm table-hover">
							...
							<c:forEach var="funcVO" items="${funcSvc.all}">
								<tr>
									<td><input type="checkbox" name="funcid"
										value="${funcVO.funcid}"
										<c:forEach var="adminFuncVO" items="${list}">
									${(funcVO.funcid==adminFuncVO.funcid)?'checked':'' }</c:forEach>>
									</td>
						...
						<button type="submit" class="btn" name="action"
							value="update_AdminFunc" style="color:#7f70f5; border-color:#7f70f5">送出修改</button>
						<button type="reset" class="btn" style="color:#7f70f5; border-color:#7f70f5">重設</button>
					</form>
				</div>
		    </div>
