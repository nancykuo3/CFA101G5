...
<body id="page-top">
    <div id="wrapper">
        ...
        <div class="d-flex flex-column" id="content-wrapper">
		    ...
		    <div style="padding:20px 15px;">
		    	<div style="display: flex;">
					<div style="display: flex; position: absolute; right:15px;">
						<h4>
							<a href="<%=request.getContextPath()%>/back-end/index-admin.jsp">回到後台首頁</a>
						</h4>
					</div>
		    	</div>
				...
				<div>
					<table class="table table-striped table-sm table-hover">
						<tr>
							<th scope="col" style="width:8%">權限編號</th>
							<th scope="col">名稱</th>
							<th scope="col">說明</th>
							<!-- <th scope="col" style="width:15%">修改</th>
							<th scope="col" style="width:8%">刪除</th> -->
							<th scope="col">查詢該權限管理員</th>
						</tr>
						<c:forEach var="funcVO" items="${funcSvc.all}">
							<tr>
								<td>${funcVO.funcid}</td>
								<td>${funcVO.funcName}</td>
								<td>${funcVO.funcdes}</td>
								<%-- <td>
									<form method="post"
										action="<%=request.getContextPath()%>/function/UpdateFuncServlet">
										<input type="hidden" name="funcid" value="${funcVO.funcid}">
										<button type="submit" name="action" value="updateOneFunc"
											disabled="disabled" class="btn back-end-btn">修改 (預設鎖住)</button>
									</form>
								</td> --%>
							<%-- 	<td>
									<form method="post"
										action="<%=request.getContextPath()%>/function/DeleteFuncServlet">
										<input type="hidden" name="funcid" value="${funcVO.funcid}">
										<button type="submit" name="action" value="deleteOneFunc"
											disabled="disabled" class="btn back-end-btn">刪除</button>
									</form>
								</td> --%>
								<td>
									<form method="post"
										...
										<input type="hidden" name="funcid" value="${funcVO.funcid}">
										<button type="submit" name="action" value="listAdminsByFunc_B" class="btn back-end-btn">送出查詢</button>
									</form>
								</td>
							</tr>
						</c:forEach>
			...
			<%if (request.getAttribute("listAdminsByFuncid") != null) {%>
			<jsp:include page="listAdminsByFuncid.jsp" />
			<%
			}
			%>
