...
<%
// Comes from UpdateAdminServlet.java
// Stores object -- AdministratorVO, including admin objects from model
// but also wrong admin objects when the input had an error.
		    ...
		    <div style="margin:20px 15px;">
		    	<div style="display:flex;">
					...
					<div style="position:absolute; right:15px;">
						<h4>
							<a href="<%=request.getContextPath()%>/back-end/admin/listAllAdmin.jsp">取消修改，回到管理員清單</a>
						</h4>
					</div>
		    	</div>
				<h3>請於下方修改資料：</h3>
				<div>
					<form method="post" action="UpdateAdminServlet" name="formUpdate">
						<table class="table table-striped table-sm table-hover">
							...
							<tr>
								<td>管理員帳號：</td>
								<td><input type="email" name="adminacc"
									value="<%=adminVO.getAdminacc()%>"></td>
							</tr>
							<tr>
								<td>管理員密碼：</td>
								<td><input type="text" name="adminpwd"
									value="<%=adminVO.getAdminpwd()%>"></td>
							</tr>
							<tr>
								<td>管理員名稱：</td>
							...
							<tr>
								...
								<td><label> <input type="radio" name="adminStatus"
										value="1" checked>有效
								</label> <label> <input type="radio" name="adminStatus" value="0">失效
								</label></td>
							</tr>
						</table>
						<input type="hidden" name="adminid" value="<%=adminVO.getAdminid()%>">
						<button type="submit" name="action" value="update" class="btn back-end-btn">送出修改</button>
					</form>
				</div>
