		    ...
		    <div style="padding:20px 15px;">
		    	<div style="display: flex;">
					...
					<div style="position:absolute;right:15px;">
						<h4>
							<a href="<%=request.getContextPath()%>/back-end/back-end-index.jsp">回首頁</a>
						</h4>
					</div>
		    	</div>
				...
				<button type="button" data-bs-toggle="modal" class="btn back-end-btn" style="color:#7f70f5; border-color:#7f70f5" 
					data-bs-target="#staticBackdrop">新增商品類型</button>
				...
				<div>
					<table class="table table-striped table-sm table-hover">
						<tr>
							...
							<th scope="col">類型分類</th>
							<th scope="col">修改</th>
							<th scope="col">查詢</th>
						</tr>
						<%@ include file="/back-end/product/pages/pageTop.file"%>
						<c:forEach var="typeVO" items="${list}" begin="<%=pageIndex%>"
							end="<%=pageIndex+rowsPerPage-1%>">
							<tr>
								<td>${typeVO.typeid}</td>
								<td>${typeVO.typeName}</td>
								<td>${typeVO.typeClass}</td>
								<td>
									<form method="post"
										action="<%=request.getContextPath()%>/productType/UpdateTypeServlet">
										<!--送出本網頁的路徑給Controller-->
										...
										<button type="submit" name="action" class="btn back-end-btn" style="color:#7f70f5; border-color:#7f70f5" 
											value="getOne_Type_For_Update">修改</button>
									</form>
								</td>
								<td>
									<form method="post"
										action="<%=request.getContextPath()%>/productInfo/ListProdInfoByTypeidServlet">
										<!--送出本網頁的路徑給Controller-->
										<input type="hidden" name="requestURL"
											value="<%=request.getServletPath()%>">
										<!--送出當前是第幾頁給Controller-->
										...
										<input type="hidden" name="typeid" value="${typeVO.typeid}">
										<button type="submit" name="action" class="btn back-end-btn" style="color:#7f70f5; border-color:#7f70f5" 
											value="listProdInfoByTypeid_B">送出查詢</button>
									</form>
								</td>
							</tr>
						</c:forEach>
				...
				<%
				if (request.getAttribute("listProdInfoByTypeid") != null) {
				%>
				<jsp:include page="/back-end/product/listProdInfoByTypeid.jsp" />
				<%
				}
				%>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="staticBackdropLabel">新增商品類型</h5>
